...
SET @book_status := (SELECT status FROM Book WHERE book_id = 113);

IF @book_status IS NULL OR @book_status <> 'available' THEN
  ROLLBACK;
  SELECT '借閱失敗：書籍不存在或已被借出' AS message;
ELSE
  
  INSERT INTO Borrow (borrow_id, user_id, book_id, borrow_date, due_date)
  VALUES (999, 14, 113, CURDATE(), DATE_ADD(CURDATE(), INTERVAL 14 DAY));

  COMMIT;
  SELECT '借閱成功！' AS message;
END IF;
